...
No rows were dropped because there are still 4177 rows like there were
...
```{R}
cols <- pivot_longer(df,-sex, names_to = "variable", values_to = "value")

ggplot(cols, aes(x = value)) +
  geom_histogram() +
...
ggplot(df, aes(x = sex, y = length)) + 
  geom_boxplot() + 
  labs(title = "Length vs. Sex Boxplot")
```

```{R}
ggplot(df, aes(x = sex, y = diameter)) + 
  geom_violin() + 
  labs(title = "Length vs. Sex Violin-plot")
```

From the analysis, it seems like females are very slightly larger in diameter than males on average.
...
```{R}
ggplot(df, aes(x = length, y = diameter, color = sex, size = shell_weight)) +
  geom_point() +
  labs(title = "Length vs. Diameter vs. Sex vs. Shell Weight")
```

The main anomaly is the point around (0.19,0.38), there are other small ones around (0.6, 0.38) and (0.29, 0.12). Also, as you increase in length and diameter the shell weight also increase increases.
...
ggplot(df, aes(x = length, y = diameter, color = sex)) + 
  ...
  labs(title = "Length vs. Diameter vs. Sex")
```
...
new_abalone <- pivot_longer(long_abalone, -sex, names_to = 'variable', values_to = 'mean_value')

ggplot(new_abalone, aes(x = variable, y = mean_value, fill = sex)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  labs(title = "Mean Values by Sex")
```
...
df %>%
  mutate(num_rings = case_when(
    rings < 10 ~ "low", rings > 20 ~ "high", TRUE ~ "med")) %>%
  ...
  geom_tile() +
  labs(title = "Num Rings vs. Avg Weight")
```
...
scatter_plots <- 
  map2(quantitative_vars, seq_along(quantitative_vars), function(var, i) {
  ggplot(df, aes_string(x = "rings", y = var, color = "sex")) +
    geom_point() +
    labs(title = var) })

grid_plot <- plot_grid(plotlist = scatter_plots, nrow = 3)
...
The intercept: when the diameter is 0, the height is -0.003803 (meaning
that the diameter can't be 0 obviously). The diameter: for every 1
increase in diameter the height increases by 0.351376. The p value: is
extremely statistically significant because it is less than 0.05
...
Yes because the data points increase at a constant rate with minimal
outliers.
